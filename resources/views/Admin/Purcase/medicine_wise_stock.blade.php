@extends('Admin.index')
@section('title','Medicine Wise Report')
@section('breadcrumbs','Medicne Wise Report')
                ...
                <div class="row">
                  ...
                  <table id="bootstrap-data-table" class="table table-striped table-bordered">
                    ...
                    @foreach($report_data as $report_data_value)
                      <tr>
                        <td>{{date('d-M-Y',strtotime($report_data_value->date))}}</td>
                        <td>{{$report_data_value->company_name}}</td>
                        <td>
                         @php
                           $medicine_data=DB::table('medicine')->where('medicine_code',$report_data_value->medicine_code)->first();
											   @endphp
                      ...
                      @endforeach
												...
												<td colspan="7" class="text-center" style="font-size: 40px;color: green;">
													Total Stock : {{$stock_data ? $stock_data->total_stock :'NOT PURCASE YET'}}
												</td>
                  </table>
                        </div>
                    </div>
